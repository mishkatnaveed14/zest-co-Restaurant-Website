...
const fallbackDishes = [
  ...
  },
  {
    ...
    rating: 4.8,
    published: true,
    image: asset("golden-fried-chicken.jpg"),
  ...
  },
  {
    ...
    rating: 4.8,
    published: true,
    image: asset("artisan-pepperoni-pizza.jpg"),
...
function renderChart() {
  ...
  $("categoryLegend").innerHTML = counts
    ...
    .map(
      (item, index) =>
        `<span class="legend-item"><i style="background:${colors[index]}"></i>${safe(item.category)} <strong>${item.count}</strong></span>`,
    )
    .join("");
}
...
async function loadFirebase() {
  try {
    ...
    if (!snapshot.empty) {
      dishes = snapshot.docs.map(normalize);
      fillCategories();
      render();
    }
    onSnapshot(collection(db, "menu"), (live) => {
      if (!live.empty) {
        dishes = live.docs.map(normalize);
        fillCategories();
        render();
      }
    });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (localStorage.getItem("zestco-theme") === "dark")
