...
function setupEventListeners() {
  document
    ...
    .addEventListener("click", () => switchView("inventory"));
  document
    .getElementById("btn-purchase")
    .addEventListener("click", () => switchView("purchase"));
  document
    .getElementById("sidebarInventoryLink")
    ?.addEventListener("click", (event) => {
      event.preventDefault();
      switchView("inventory");
    });
  document
    .getElementById("sidebarPurchaseLink")
    ?.addEventListener("click", (event) => {
      event.preventDefault();
      switchView("purchase");
    });
  ...
  document
    ...
    .addEventListener("keyup", handleFilter);
  document
  ...
  function openModal() {
    ...
    document.getElementById("modalTitle").innerText =
      editingInventoryId === null
        ...
        : "Update Stock";
    document.getElementById("itemName").previousElementSibling.innerText =
      isInventory ? "Item Name" : "Supplier";
    document.getElementById("itemName").placeholder = isInventory
      ? "e.g. Fresh Salmon"
      : "e.g. Ocean Catch Ltd";
    document.getElementById("itemQtyLabel").innerText = "Quantity";
    document.getElementById("itemMaxLabel").innerText = isInventory
      ? "Reorder Point"
      : "Reference Number";
    document.getElementById("itemMax").placeholder = isInventory
      ? "e.g. 50"
      : "Optional";
    ...
    if (!isInventory) {
      document.getElementById("itemDate").value = new Date()
        .toISOString()
        .split("T")[0];
    }
    ...
    inventoryAnimationApi.to(modalCard, {
      duration: 0.4,
      scale: 1,
      ease: "back.out(1.7)",
    });
  ...
  window.openStockEditor = (itemId) => {
    const item = inventoryData.find(
      (inventoryItem) => String(inventoryItem.id) === String(itemId),
    );
    if (!item) return;
  ...
  document
    .getElementById("closeDetailsFooterBtn")
    .addEventListener("click", closeDetails);
    ...
    .addEventListener("submit", async (e) => {
      ...
      try {
        if (currentView === "inventory" && editingInventoryId !== null) {
          const item = inventoryData.find(
            (inventoryItem) =>
              String(inventoryItem.id) === String(editingInventoryId),
          );
          if (item) {
            item.qty = qty;
            item.reorder = maxQty;
          ...
          const itemReference = await addDoc(
...
function switchView(view) {
  ...
  document
    .getElementById("btn-inventory")
    .classList.toggle("active", view === "inventory");
  document
    .getElementById("btn-purchase")
    .classList.toggle("active", view === "purchase");
  document
    .getElementById("sidebarInventoryLink")
    ?.classList.toggle("active", view === "inventory");
  document
    .getElementById("sidebarPurchaseLink")
    ?.classList.toggle("active", view === "purchase");
  document.getElementById("actionButtonText").innerText =
    ...
    paginatedData.forEach((item) => {
      ...
      if (currentView === "inventory") {
        const percentage = Math.min(
          Math.round((item.qty / item.maxQty) * 100),
          100,
        );
...
function initCharts() {
  ...
  const supplyCtx = document
    .getElementById("supplyOverviewChart")
    .getContext("2d");
  ...
  document.getElementById("stockRange").addEventListener("change", (event) => {
    ...
    document.getElementById("stockMetric").innerText = values.reduce(
