...
const mockReviews = [
    {
        ...
        comment: "Absolute perfection! The biryani was incredibly rich in spices and arrived piping hot. Served with fresh green chutney raita. Will definitely order again!",
        createdAt: { seconds: Math.floor(Date.now() / 1000) - 7200 },
        status: "Approved",
        adminReply: "Thank you so much Saima! We are delighted to hear you enjoyed our special biryani!"
    },
    {
        id: "REV-102",
        ...
        comment: "Great food overall. The korma was delicious with fresh naans. Slight delay in delivery by 10 minutes though.",
    ...
    },
    {
        ...
        comment: "The pakodas were amazingly crispy and delicious! Tasted like authentic homemade food.",
    ...
    },
    {
        ...
        comment: "Food was warm but spice level was far lower than requested. Expected it to be much spicier.",
...
function listenToReviews() {
    ...
    onSnapshot(reviewsRef, (snapshot) => {
        ...
        console.warn("Firestore access offline/empty. Rendering mock data array.", error);
        globalReviews = [...mockReviews];
        updateMetrics(globalReviews);
        renderReviews();
    });
...
function updateMetrics(reviews) {
    ...
    const avg = (reviews.reduce((acc, r) => acc + (r.rating || 5), 0) / total).toFixed(1);
...
function renderReviews() {
    ...
    const filtered = globalReviews.filter(rev => {
        const matchesSearch = (rev.customerName && rev.customerName.toLowerCase().includes(searchTerm)) ||
                              (rev.comment && rev.comment.toLowerCase().includes(searchTerm)) ||
                              (rev.dishOrdered && rev.dishOrdered.toLowerCase().includes(searchTerm));
        const matchesStar = activeStarFilter === "All" || rev.rating == activeStarFilter;
    ...
    reviewsContainer.innerHTML = filtered.map(rev => {
        const dateStr = rev.createdAt ? new Date(rev.createdAt.seconds * 1000).toLocaleDateString() : 'N/A';
...
function openReplyModal(id) {
    ...
    document.getElementById('replyModalCustomer').textContent = `Reply to ${rev.customerName}`;
    document.getElementById('replyOriginalAuthor').textContent = `Customer: ${rev.customerName}`;
...
document.querySelectorAll('#starFilterButtons .btn-filter').forEach(btn => {
    btn.addEventListener('click', () => {
        document.querySelectorAll('#starFilterButtons .btn-filter').forEach(b => b.classList.remove('active'));
